...
function serve(host: string, other: () => string): Promise<{ server: Server; origin: string }> {
...
export async function startFixture(): Promise<Fixture> {
  const origins = { a: '', b: '' }
  const a = await serve('127.0.0.1', () => origins.b)
  const b = await serve('localhost', () => origins.a)
  origins.a = a.origin
  origins.b = b.origin
