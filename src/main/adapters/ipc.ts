 ...
 *
 * Every call from here is named as the window's, which is the developer acting: this is
 * the surface the origin allow-list deliberately does not bind ([ADR-0013]).
 */
export function registerIpcAdapter(dispatch: Dispatch): void {
  ipcMain.handle(ROUTE_CHANNEL, async (_event, raw: unknown): Promise<RouteResponse> => {
    ...
    const { response, afterRespond } = await dispatch(parsed.request, { surface: 'window' })
